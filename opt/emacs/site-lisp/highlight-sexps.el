...
(defun hl-sexp-highlight ()
  ...
  (unless (= (point) hl-sexp-last-point)
    ...
    ;; (let ((overlays hl-sexp-overlays) ; jm
    ;;       (sexp-list (hl-sexp-end-points (point) ; jm
    ;;                                      (length hl-sexp-overlays))) ;jm
    (let ((overlays (reverse hl-sexp-overlays))
          (sexp-list (reverse (hl-sexp-end-points (point) 
...
;;  jm - may need to be fixed for non-lisp code
...
(defun hl-sexp-end-of-sexp (pt)
  ...
  (condition-case ()
      (scan-sexps pt 1)
    (error (progn nil))))   ;; jm version swallows errors
